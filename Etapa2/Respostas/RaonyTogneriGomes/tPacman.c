...
void MovePacman(tPacman* pacman, tMapa* mapa, COMANDO comando) {
    ...
    if (PossuiTunelMapa(mapa)) {
        if (EntrouTunel(ObtemTunelMapa(mapa), pacman->posicaoAtual) && !(pacman->bateuParede)) {
            AtualizaTrilhaPacman(pacman);
